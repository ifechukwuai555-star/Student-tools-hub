...
function calculateGPA() {
  ...
  for (let i = 0; i < courses.length; i++) {
    ...
    if (!name) {
      showError(
        `Please enter the name for course ${i + 1}.`
      );
      return;
    }
    ...
    if (unitsText === "") {
      showError(
        `Please enter the credit units for course ${i + 1}.`
      );
      return;
    }
    ...
    if (!Number.isFinite(units) || units <= 0) {
      showError(
        `Credit units for course ${i + 1} must be greater than 0.`
      );
      return;
    }
    ...
    if (!Number.isInteger(units)) {
      showError(
        `Credit units for course ${i + 1} must be a whole number.`
      );
      return;
    }
